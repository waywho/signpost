...
class ActionItems::ActionsControllerTest < ActionDispatch::IntegrationTest
  setup do
  ...
  test "create creates delegation and marks actioned" do
    assert_difference("Delegation.count") do
      post action_item_action_path(@item)
    end
  ...
  test "create with developer assigns delegation" do
    dev = create(:developer)
    post action_item_action_path(@item), params: { developer_id: dev.id }
    delegation = Delegation.last
    assert_equal dev, delegation.developer
    assert_equal dev.name, delegation.developer_name
  end

  test "create with existing issue URL stores it" do
    post action_item_action_path(@item), params: { existing_issue_url: "https://github.com/org/repo/issues/42" }
    @item.reload
  ...
  test "create with notify_thread and developer posts message" do
    dev = create(:developer, slack_handle: "U12345")
    calls = with_mock_slack do
      post action_item_action_path(@item), params: { developer_id: dev.id, notify_thread: "1" }
    end
    assert_redirected_to root_path
  ...
  test "create with notify_thread and ticket posts message with link" do
    dev = create(:developer, slack_handle: "U12345")
    calls = with_mock_slack do
      post action_item_action_path(@item), params: {
  ...
  test "create with notify_thread no developer no ticket adds emoji" do
    calls = with_mock_slack do
      post action_item_action_path(@item), params: { developer_id: "", notify_thread: "1" }
    end
    assert_equal 1, calls[:add_reaction].size
    assert_equal "eyes", calls[:add_reaction].first[:emoji]
  end

  test "create with notify_thread no developer but ticket posts ticket message" do
    calls = with_mock_slack do
      post action_item_action_path(@item), params: {
